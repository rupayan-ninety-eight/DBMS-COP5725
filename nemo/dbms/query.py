...
def query_1(query1_area_code):
    ...
    return (
        ...
        FROM crime c, 
            (
                SELECT t1.calender_year AS calender_year, 
                    t1.crime_code AS crime_code, 
                    ROUND(((t1.no_of_incidents - t2.no_of_incidents) / t2.no_of_incidents) * 100, 2) AS yearly_growth,
                    t2.calender_year AS previous_year, 
                    t1.no_of_incidents AS Current_Year_Incidents, 
                    t2.no_of_incidents AS Prev_Year_Incidents
                FROM (
                        SELECT 
                ...
                ON t1.crime_code = t2.crime_code 
                    AND t2.calender_year = t1.calender_year - 1
                WHERE (t1.calender_year, ROUND(((t1.no_of_incidents - t2.no_of_incidents) / t2.no_of_incidents) * 100, 2)) IN 
                    (
                        SELECT t1.calender_year, 
                            MAX(ROUND(((t1.no_of_incidents - t2.no_of_incidents) / t2.no_of_incidents) * 100, 2))
                        FROM (
                                SELECT 
                                    EXTRACT(year from DATE_TIME) AS calender_year,
                                    crime_code,
                        ...
                        ON t1.crime_code = t2.crime_code 
                            AND t2.calender_year = t1.calender_year - 1
                        WHERE ROUND(((t1.no_of_incidents - t2.no_of_incidents) / t2.no_of_incidents) * 100, 2) IS NOT NULL
...
def query_2(query2_bureau):
    ...
    query2 = f"""
    SELECT CONCAT( CONCAT( arr.calender_year, '-Q' ), arr.calender_quarter ) AS YEAR_QUARTER, 
        ROUND((arr.no_of_arrests / inc.no_of_incidents) * 100, 2) AS Arrest_Percentage
    FROM
        (
            SELECT 
                EXTRACT(year from DATE_TIME) AS calender_year,
                TO_CHAR(Date_Time, 'Q') AS calender_quarter,
                count(DISTINCT(DR_NO)) AS no_of_arrests
            FROM (
                    SELECT i.dr_no as dr_no, 
            ...
            GROUP BY EXTRACT(year from DATE_TIME), TO_CHAR(Date_Time, 'Q')
            ORDER BY calender_year ASC
        ) arr,
        (
            SELECT 
...
def query_3(ethnicity, crime_codes):
    ...
    SELECT p.premises_desc AS premise_desc, 
        c.time_hour AS time_hour, 
        c.no_of_incidents AS no_of_incidents
    FROM premises p, (
                        SELECT premises_code, 
                            EXTRACT(HOUR from DATE_TIME) AS time_hour, 
                            COUNT(DISTINCT(dr_no)) AS no_of_incidents
...
def query_4(query4_AGE_RANGE, query4_year):
    ...
    query4 = f"""
        ...
        FROM district d,
            ...
            FROM (
                SELECT t1.area_code AS area_code, 
                    ROUND(((t2.total_incidents - t1.total_incidents)/t1.total_incidents) * 100, 2) AS GROWTH_Percentage 
                FROM   (
                        SELECT * FROM (
                                        SELECT area_code, 
                                            calender_year, 
                                            no_of_incidents, 
                                            SUM(no_of_incidents) OVER (PARTITION BY area_code ORDER BY calender_year) AS total_incidents
                                        FROM (
                                                SELECT area_code, 
                                                    EXTRACT(YEAR FROM DATE_TIME) AS calender_year, 
                                                    COUNT(DISTINCT(dr_no)) AS no_of_incidents
                ...
                WHERE t1.area_code = t2.area_code 
                    AND t1.calender_year = {query4_year} 
                    AND t2.calender_year = 2019  
                ) c
            ) ar
...
def query_5(query5_year_range):
    ...
    query5 = f"""
    SELECT t1.calender_year, 
        ROUND((t1.male_incidents/t2.female_incidents) * 100, 2) AS MaleToFemaleRatio
    FROM (
        SELECT EXTRACT(YEAR FROM i.DATE_TIME) AS calender_year, 
            COUNT(DR_NO) AS male_incidents
        FROM incidents i, victims v
        ...
        (
        SELECT EXTRACT(YEAR FROM i.DATE_TIME) AS calender_year, 
            COUNT(DR_NO) AS female_incidents
        FROM incidents i, victims v
        WHERE v.victim_id = i.victim_id 
            AND v.gender = 'F'
            AND i.weapon_code IS NOT NULL 
